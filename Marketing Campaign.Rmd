...
-   Fase 5 - Tarea 2: Análisis estadístico

**Iván:**

-   Fase 1: Exploración de los datos y análisis preliminar

-   Fase 2: Muestreo y modificación de los datos

-   Fase 5 - Tarea 1: Análisis exploratorio de los datos

-   Fase 5 - Tarea 3: Visualización de los datos

-   Fase 5 - Tarea 4: Recomendaciones y conclusiones
...
```{r}
# Cargamos el dataset
marketing_bruto <- read_delim(file = "/Users/leztin/Desktop/DATOS/marketing_campaign.csv", delim = ";")
```
...
Lo que haremos será restar el año actual, **2023**, al año de nacimiento del individuo (`Year_Birth`):
...
Por este motivo, se ha decidido **eliminar** este tipo de variables.
...
Otro problema que suele suceder es la **fuerte correlación que muchas veces se presenta entre las variables**: si tomamos demasiadas variables (cosa que en general sucede cuando no se sabe demasiado sobre los datos o sólo se tiene ánimo exploratorio), lo normal es que estén **relacionadas o que midan lo mismo bajo distintos puntos de vista**.
...
Si observamos los resultados, los tres primeros componentes principales explican el **74.85 % de la variación en los datos**.
...
Como queremos cumplir el **método de la varianza explicada**, retendremos finalmente los tres primeros factores, explicando con ellos el **74.85 % de la variabilidad total de nuestros datos**.
...
Para comprobar cuáles son las variables **más relacionadas** con el **número de compras en tienda** (`NumStorePurchases`) podemos analizar el ACP que lanzamos en fases anteriores.

```{r}
a1 <-  marketing_rank |> 
  mutate(PC1= componentes$PC1 , PC2 = componentes$PC2, 
         PC3 =componentes$PC3, PC4=componentes$PC4) |> 
  cor() |> as_tibble(rownames = NA) |> tail(n = 4)  |> 
  select(NumStorePurchases) |> 
  cbind(Componentes = c("Componente 1", "Componente 2", 
                        "Componente 3", "Componente 4")) |> 
  select(Componentes, everything())
a1
```

Tal y como se muestra en la tabla de salida, podemos observar cómo `NumStorePurchases` mantiene **correlaciones muy elevadas** con la **Componente 1**.
Recordemos que esta componente representaba a todas aquellas variables que estaban relacionadas con el **nivel económico del cliente** (gastos, ingresos, compras, etc.).
De esta manera, podemos intuir cómo esta variable mantendrá **correlaciones altas** con variables del tipo `Income`, `NumWebPurchases` o `TotSpent`.

Si nos fijamos directamente en la **tabla de correlaciones**, podremos observar como efectivamente las variables más correlacionadas son `Income`, `Wines`, `Meat`, `NumWebPurchases`, `NumCatalogPurchases` o `TotSpent`.
Todas ellas cuantifican el **nivel de gasto**, el **nivel económico** o el **total de compras del cliente**.
Como es normal, el cliente que tienda a comprar más en establecimientos físicos comprará menos en el resto de canales a través de los cuales la empresa distribuya sus productos.

### ¿A Estados Unidos le va significativamente mejor que al resto del mundo en términos de compras totales?

Como no habíamos incluido esta variable en nuestro dataset (no venía incluida en las instrucciones de la tarea), **la incluiremos ahora para responder a estas preguntas**.

```{r}
# Cargamos el fichero de marketing que contiene la columna `Country`
marketing_paises  <- read_delim(file = "/Users/leztin/Desktop/DATOS/marketing_data.csv", delim = ",")

# Añadimos la columna `Country` a nuestro fichero original
...
Para responder a la pregunta, crearemos una gráfica donde visualizaremos las **compras totales en función de los distintos países**.

```{r}
# Agrupar¡mos el dataframe por `Country` y ordenamos por el número de compras
marketing_paises <- marketing |> 
  group_by(Country) |> 
  arrange(desc(TotalPurchases))

# Gráfico de barras de país según el numero de compras totales
marketing_paises |> 
  ggplot(aes(x = TotalPurchases, y = reorder(Country, TotalPurchases))) + 
  geom_bar(stat = "identity", fill = "#56BCC2") + 
  ggtitle("Numero total de compras por país") +
...
Si observamos el gráfico de barras, podremos comprobar como es **España el país al que mejor le va en términos de compras totales**.
Estados Unidos se encuentra situado en la **penúltima posición**.

```{r}
# Gráfico de barras de país según el numero de gastos totales
marketing_paises |> 
  ggplot(aes(x = TotSpent, y = reorder(Country, TotSpent))) + 
  geom_bar(stat = "identity", fill = "#56BCC2") + 
  ggtitle("Gasto total por país") +
  xlab("Gasto total") + 
  ylab("País") +
  theme_minimal()  +
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
...
En cuanto al dinero gastado (`TotSpent`), vemos que igualmente **España es el país que encabeza la lista**, mientras que **Estados Unidos sigue siendo el penúltimo**.

**Por tanto, podemos concluir con que a Estados Unidos no le va mejor que al resto de países en términos de compras totales.**
...
```{r}
  marketing_paises |> 
  ggplot(aes(x = Gold, y = NumStorePurchases)) +
  geom_point(col = "#56BCC2") + 
  geom_smooth(method = "lm", se = FALSE, color = "black", aes(group = 1)) +
  theme_minimal() + 
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5))
...
Si observamos el gráfico, veremos que el número de compras en tienda (`NumStorePurchases`) y el gasto en oro (`Gold`) mantienen una **correlación positiva**, por lo que sí podríamos afirmar que **las personas que compran oro suelen comprar más en las tiendas físicas**.

```{r}
kendall_corr <- 
  cor(marketing_paises$Gold, marketing_paises$NumStorePurchases, method = 'kendall')
kendall_pvalue <- 
  cor.test(marketing_paises$Gold, marketing_paises$NumStorePurchases, method = 'kendall')
...
Si realizamos sobre estas dos variables el **test de correlaciones de Kendall** veremos que ofrece un **p-valor inferior a 0.05**, por lo que no podemos aceptar la hipótesis nula y, por tanto, confirmamos que **ambas variables mantienen una correlación positiva estadísticamente significativa**.
...
Para ello primeramente crearemos dos tablas: una para los clientes que están **casados y son doctores**; y otra con los clientes que **no los son**.

```{r}
marketing_bruto_casadoctorado <- 
  marketing_bruto|> filter(Marital_Status== "Married" & Education =="PhD")

marketing_bruto_casadoctoradono <- 
  marketing_bruto|> filter(!(Marital_Status== "Married" & Education =="PhD"))
```

De esta forma los podremos comparar mediante una gráfica y comprobar si realmente estas características del cliente influyen en la compra de pescados.

```{r layout="l-body-outset", fig.width=13, fig.asp = .7}
# Gráfica de la compra de pescado de los casados y doctorados
b1 <- marketing_bruto_casadoctorado |> 
  ggplot(aes(x= MntFishProducts)) +
  geom_boxplot(col = "#56BCC2") +
  theme_minimal() + 
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) +
  ggtitle("Gasto en pescados por clientes casados y doctorados")

# Gráfica de la compra de pescado de los no que son a la vez casados y doctorados
b2 <- marketing_bruto_casadoctoradono |> 
  ggplot(aes(x = MntFishProducts)) +
  geom_boxplot(col = "#56BCC2") +
  theme_minimal() + 
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) +
  ggtitle("Gasto en pescados por clientes no casados ni doctorados")

Rmisc::multiplot(b1, b2)
```

Comparando ambas gráficas, **no se puede afirmar que aumente la compra de pescado por estar el cliente casado y doctorado**.
De hecho, la mediana del gasto en pescado de los casados doctorados se ve **reducida**.

Para comprobar esta cuestión numéricamente vamos a utilizar un **test de diferencia significativa** entre estos dos grupos en términos de media.

```{r}
pval <- 
  t.test(marketing_bruto_casadoctorado$MntFishProducts, 
         marketing_bruto_casadoctoradono$MntFishProducts)$p.value
cat("t-test p-value: ", round(pval, 3))
```

Si observamos el resultado, veremos que ofrece un **p-valor inferior a 0.05**, por lo que no podemos aceptar la hipótesis nula y, por tanto, confirmamos que **ambas variables son diferentes en términos de media**.
En definitiva, los clientes que no están casados y que no son doctores tienden a gastar más en productos de pescado que los que sí lo están.

### Responda al siguiente enunciado (III):
...
Nuevamente podemos recurrir a nuestro ACP realizado en fases anteriores.
...
Tal y como se muestra en la tabla de salida, podemos observar cómo `Fish` mantiene **correlaciones muy elevadas** con la **Componente 1**.
Recordemos que esta componente representaba a todas aquellas variables que estaban relacionadas con el **nivel económico del cliente** (gastos, ingresos, compras, etc.).
De esta manera, podemos intuir cómo esta variable mantendrá **correlaciones altas** con variables del tipo `Income`, `NumCatalogPurchases` o `TotSpent`.

Si nos fijamos directamente en la **tabla de correlaciones**, podremos observar como efectivamente las variables más correlacionadas son `Fruits`, `Wines`, `Meat`, `Sweets`, `Gold`, `NumCatalogPurchases` o `TotSpent`.
Todas ellas cuantifican el **nivel de gasto**, o el **total de compras del cliente**.
En este sentido, parece que los clientes que gastan más en total (`TotSpent`) tienden a gastar también más en el sector de los pescados (`Fish`).
Además, también parece que `Fish` **correlaciona de manera negativa con el resto de sectores de la empresa**, **de manera que quien compra más pescados, tiende a comprar menos en el resto de sectores**.
...
```{r}
# Cargamos el fichero de marketing que contiene la columna `Country`
marketing_exito  <- read_delim(file = "/Users/leztin/Desktop/DATOS/marketing_data.csv", delim = ",")
...
Una vez almacenados estos datos, los mostraremos en gráficas para poder visualizarlos y entenderlos de una manera más rápida y sencilla.

```{r layout="l-body-outset", fig.width=13, fig.asp = .9}
b1 <- porcentaje1 |> 
  ggplot(aes(x = Country, y =por)) +
  geom_bar(stat='identity' ,alpha = .8, fill="#56BCC2") +
  labs(title = "Porcentaje de clientes por países que \nhan aceptado la primera campaña", x = "País", y = NULL) +
  theme_minimal() +
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) + 
  scale_linetype_manual(name = "Medidas", values = c(Media = "solid", Mediana = "dotted")) 
...
b2 <- porcentaje2 |> 
  ggplot(aes(x = Country, y=por)) +
  geom_bar(stat='identity' ,alpha = .8, fill="#56BCC2") +
  labs(title = "Porcentaje de clientes por países que \nhan aceptado la segunda campaña", x = "País", y = NULL) +
  theme_minimal() +
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) + 
  scale_linetype_manual(name = "Medidas", values = c(Media = "solid", Mediana = "dotted")) 
...
b3 <- porcentaje3 |> 
  ggplot(aes(x = Country, y=por)) +
  geom_bar(stat='identity' ,alpha = .8, fill="#56BCC2") +
  labs(title = "Porcentaje de clientes por países que \nhan aceptado la tercera campaña", x = "País", y = NULL) +
  theme_minimal() +
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) + 
  scale_linetype_manual(name = "Medidas", values = c(Media = "solid", Mediana = "dotted")) 
...
b4 <- porcentaje4 |> 
  ggplot(aes(x = Country, y=por)) +
  geom_bar(stat='identity' ,alpha = .8, fill="#56BCC2") +
  labs(title = "Porcentaje de clientes por países que \nhan aceptado la cuarta campaña", x = "País", y = NULL) +
  theme_minimal() +
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) + 
  scale_linetype_manual(name = "Medidas", values = c(Media = "solid", Mediana = "dotted")) 
...
b5 <- porcentaje5 |> 
  ggplot(aes(x = Country, y=por)) +
  geom_bar(stat='identity' ,alpha = .8, fill="#56BCC2") +
  labs(title = "Porcentaje de clientes por países que \nhan aceptado la quinta campaña", x = "País", y = NULL) +
  theme_minimal() +
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) + 
  scale_linetype_manual(name = "Medidas", values = c(Media = "solid", Mediana = "dotted")) 
...
b6 <- porcentajefinal |> 
  ggplot(aes(x = Country, y=por)) +
  geom_bar(stat='identity' ,alpha = .8, fill="#56BCC2") +
  labs(title = "Porcentaje de clientes por países que \nhan aceptado la última campaña", x = "País", y = NULL) +
  theme_minimal() +
  theme(text = element_text(face = "bold", size = 15), plot.title = element_text(hjust = 0.5), 
        axis.title.x = element_text(vjust=-0.5)) + 
  scale_linetype_manual(name = "Medidas", values = c(Media = "solid", Mediana = "dotted")) 
...
Observando los gráficos de cada campaña podemos llegar a las siguientes conclusiones.
El **porcentaje de aceptación** de todas las campañas es, en general, **bastante bajo**.
La **última campaña** (`Response`) es la que ha logrado una **tasa de aceptación más alta**.
Además, el **país** que ha logrado una **mayor tasa de aceptación** en las últimas campañas es **México**, con un increíble **65 % de aceptación** en la última campaña.
En las cuatro primeras campañas México aparece en 0 probablemente porque **todavía no se habían diseñado campañas de marketing para el país**.
...
Para el análisis de la **tipología del cliente** en esta empresa, emplearemos una de las mejores segmentaciones que obtuvimos al aplicar los distintos algoritmos de agrupamiento a las características del cliente (se podría hacer perfectamente también con el resto de segmentaciones que hemos presentado en el trabajo).
...
Los clientes Star son los **más valiosos** dentro de la empresa, por lo que resulta importante mantenerlos **comprometidos y fieles a la marca**.
Algunas estrategias que se nos ocurren para poder mantenerlos podrían ser:

-   Ofrecerles un **servicio de atención al cliente** personalizado y dedicado.
-   Organizar **eventos exclusivos** para clientes VIP.
-   Ofrecerles **descuentos exclusivos y personalizados** dentro del programa de lealtad de la propia empresa.
-   Enviarles **comunicaciones personalizadas y adaptadas a sus gustos** por medio del newsletter de la empresa.
-   O, incluso, hacerles formar parte de la propia empresa pidiéndoles **retroalimentación** para mejorar continuamente los productos que más consumen y sus propios servicios.
...
Los High potential son clientes que tienen el **potencial** de convertirse en clientes Stars, por lo que resulta importante **invertir en ese proceso de transformación**.
Algunas estrategias que se nos ocurren para poder apoyarlos podrían ser:

-   Ofrecerles **pruebas gratuitas** de nuevos productos o servicios.
-   Organizar **capacitaciones especiales** para ayudarles a sacar el máximo provecho de los productos que más compran.
-   Ofrecerles **descuentos y promociones especiales** con tal de que no olviden la marca.
-   **Comunicarles las últimas novedades y actualizaciones** para que puedan seguir el devenir de la empresa.
...
Los Need attention son clientes con **mayor antigüedad**, pero con un **nivel económico más bajo** y, por lo general, un **nivel de gasto también más reducido**.
...
Para el resto de clientes **Need attention**, se proponen las siguientes estrategias:

-   Introducirles en el **programa de fidelización** para clientes antiguos, con beneficios exclusivos como descuentos, promociones, etc.
-   Ofrecerles **opciones de financiación asequibles** para ayudarles a comprar los productos o servicios de mayor precio.
-   Ofrecerles un **servicio de atención al cliente** que esté disponible para ayudarles a encontrar soluciones o opciones de reparación de productos **asequibles**.
...
Los Leaky bucket son clientes con **menor antigüedad**, con un **nivel económico más bajo** y, por lo general, un **nivel de gasto también más reducido**.
Suelen ser clientes, o bien recientes, o bien con un riesgo elevado de abandonar la empresa (About to sleep), o bien clientes de compra intermitente.
En este caso concreto, lo importante será tratar de **retenerlos** antes de que sea demasiado tarde.
Algunas estrategias que se nos ocurren para tratar de que no abandonen podrían ser:

-   Introducirles en el programa de fidelización para incentivarles con recompensas por hacer **varias compras en un periodo determinado de tiempo**.
-   Enviarles **comunicaciones personalizadas y adaptadas a sus gustos** por medio del newsletter de la empresa con tal de no perder el contacto.
-   Investigar posibles **problemas de insatisfacción** que puedan ser consecuencia del abandono de este tipo de clientes y tratar de remediarlos de algún modo.

```{r echo=FALSE}
rfm_scores
```

Todas estas estrategias deben estar **sincronizadas** con los **sistemas de puntuación RFM**, de manera que a cada cliente nuevo que entre en la empresa el sistema lo **puntúe**, lo clasifique automáticamente en el **clúster adecuado** y se comiencen a desplegar **las estrategias que le correspondan**.
**Todo con tal de personalizar la experiencia del cliente.**
